...
<p align="center">
  <img src="logo.png" alt="QuotaPeek 临时 Logo" width="128">
</p>
...
1. 打开组件右上角的设置，选择 **Hone API**，粘贴 API key，点击“测试连接”并保存。默认地址是 `https://hone.vvvv.ee`。这里是 key / 兼容账单，不一定是账户钱包。
   要显示**钱包剩余**，点击“＋ 连接钱包余额”，在 **Hone 钱包** 中填写网站「安全与访问」里的**账户访问令牌**，测试后保存。同一站点的钱包和 API key 会合并为一张 **Hone** 卡片：钱包余额优先展示，账户累计消费与 key 消费各自标注；设置中仍分别管理两种凭据。普通 API key 无法代替账户访问令牌。可以通过 `dist\QuotaPeek.exe --settings` 启动并打开设置。
...
3. 单击胶囊展开，点击面板外部收起；任务栏模式关闭展开面板，自由悬浮模式收回胶囊。悬停保持收起，滚轮向下切换下一个钱包 / 额度，向上切换上一个，按已启用的数据源顺序首尾循环；手动选择在本次运行中保持，自动刷新不会跳回。按住胶囊任意位置（包括余额文字、圆点、空白处和右侧握柄）并移动即可拖动。展开后也可拖动标题和卡片空白区域。拖动时保持当前形态，松手自动记住位置；仅移开鼠标不会收起。点击面板内部、设置和菜单不触发收起；设置窗口打开期间保持可见。
...
- 展开卡片按供应商及服务地址分组，钱包优先，其他服务紧凑显示；每项服务保留自己的金额、币种、趋势、更新时间和异常提示，不相加。不同站点 / 租户路径分开，手动录入独立展示，自定义 HTTP 按实际查询地址分组。胶囊仍可逐项切换钱包 / 额度。
...
README 和界面直接使用根目录 `logo.png` 原图，保留原始透明背景。替换后运行 `powershell -NoProfile -File tools\update-logo.ps1`，按原比例重新生成 EXE / 窗口 / 托盘使用的多尺寸 ICO，不添加底色、不修改原图。
...
python tests\provider_group_smoke.py --exe dist\QuotaPeek.exe
python tests\outside_click_smoke.py --exe dist\QuotaPeek.exe
python tests\outside_click_smoke.py --exe dist\QuotaPeek.exe --docked
```
