...
settings_path.write_text(json.dumps({'Version': 1, 'Providers': [], 'LeftPixels': 999999, 'TopPixels': 999999, 'StartExpanded': False}), encoding='utf-8')
...
for phase in range(2):
    p = subprocess.Popen([str(root/'dist/QuotaPeek.exe'), '--data-dir', str(artifact)], creationflags=subprocess.CREATE_NO_WINDOW)
    try:
        a = Application(backend='uia').connect(process=p.pid, timeout=20)
        w = a.window(title='QuotaPeek'); w.wait('visible', timeout=15)
        ...
        if phase == 0:
            ...
            position = (work.left + 40, work.top + 40)
